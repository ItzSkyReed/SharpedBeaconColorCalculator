...
* **Hybrid Pipeline:** Uses lightweight KD-Trees for 2 to 5 glass layers (finding matches in `O(log N)`), and massive precomputed LUTs for 6 to 9 layers (`O(1)`) (can be configured to works differenty).
...
4. For deeper layers (6-9) where KD-Trees aren't fast enough for bulk operations, the generator precomputes a 16.7M elements LUT by multithreading queries against the KD-Tree.
...
// This will generate KD-Trees (up to 5 layers) and LUTs (up to 9 layers),
